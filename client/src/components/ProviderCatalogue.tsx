import { Activity, CheckCircle2, CircleAlert, Database, ExternalLink, Filter, LoaderCircle, LockKeyhole, Search, ShieldAlert, ShieldCheck, ShieldOff, SlidersHorizontal } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import { toast } from "sonner";
import { apiCatalogue, catalogueCounts, officialProviderWebsite, providerPlaceholderKey, type CatalogueCategory } from "@/lib/apiCatalogue";
...
export default function ProviderCatalogue({ openIntegration, focusCategory, clearFocus }: ProviderCatalogueProps) {
  ...
  const renderCard = (category: CatalogueCategory, reviewSurface = false) => {
    ...
    return <article className={`catalogue-card ${category.reviewRequired ? "review-required" : ""} ${reviewSurface ? "review-surface-card" : ""}`} key={category.id} id={`api-category-${category.id}`}><div className="catalogue-card-top"><span className="catalogue-number">{String(apiCatalogue.indexOf(category) + 1).padStart(2, "0")}</span>{category.reviewRequired && <span className="review-badge"><ShieldAlert size={12} />Review</span>}</div><h3>{category.name}</h3><p>{category.function}</p><dl><div><dt>Owner</dt><dd>{category.owner}</dd></div><div><dt>Auth</dt><dd>{category.auth}</dd></div><div><dt>Resource</dt><dd>{state === "testing" ? "Checking safely" : state === "review" ? "Review before setup" : state === "needs-resource" ? "No resource connected" : "Placeholder only — not connected"}</dd></div></dl><details><summary><Filter size={13} /> View {category.providers.length} placeholders</summary><div className="provider-tag-list">{category.providers.map((provider) => { const website = officialProviderWebsite(provider); return website ? <a key={providerPlaceholderKey(category.id, provider)} href={website} target="_blank" rel="noreferrer" title={`Open ${provider} official setup website`}>{provider}<ExternalLink size={11} /></a> : <span key={providerPlaceholderKey(category.id, provider)}>{provider}</span>; })}</div><p className="provider-website-note">A linked name opens the provider’s official website. Arthur remains unconnected until you add and test a resource deliberately.</p></details><div className="catalogue-card-actions"><button className="text-button" onClick={openIntegration}>{category.reviewRequired ? "Review setup" : "Add resource"}</button><button className="test-button" onClick={() => testCategory(category)} disabled={state === "testing"}>{state === "testing" && <LoaderCircle size={12} className="spin" />}{state === "testing" ? "Testing safely…" : "Test room"}</button></div></article>;
  };
