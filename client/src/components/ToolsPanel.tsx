...
export default function ToolsPanel() {
  ...
  const [windowsHelloPreferred, setWindowsHelloPreferred] = useState(false);
    ...
    <section className="tools-layout">
      ...
      <section className="tools-panel spatial-workspace-review">
        ...
        <div className={`spatial-room-access ${spatialUnlocked ? "unlocked" : "locked"}`}>
          <div><span className="eyebrow">Protected room / preview only</span><h4>{spatialUnlocked ? "Unlocked for this preview session" : "Locked until local access is verified"}</h4><p>The installed Windows app stores only a salted password verifier in Windows Credential Manager. It may offer Windows Hello after you configure face or PIN in Windows Settings; Arthur never scans, retains, or uploads facial data.</p></div>
          <div className="spatial-room-actions">
            <button className="outline-button" onClick={() => { setSpatialPasswordReady(true); toast("Password setup is represented in this preview.", { description: "In the installed app, you choose and confirm a 10+ character password; its plaintext is never saved in Arthur’s configuration." }); }}>Set password fallback</button>
            <label className="review-choice"><input type="checkbox" checked={windowsHelloPreferred} onChange={(event) => setWindowsHelloPreferred(event.target.checked)} /> Offer Windows Hello after I configure it in Windows</label>
            <button className="primary-button compact" onClick={() => { if (!spatialPasswordReady) { toast.error("Set the password fallback first.", { description: "Windows Hello is optional; the installed app keeps a local password recovery method." }); return; } setSpatialUnlocked(true); toast("Protected Spatial room unlocked in preview.", { description: windowsHelloPreferred ? "The Windows app will ask Windows Hello or the password; this browser preview does neither." : "The Windows app will ask the local password; this browser preview does not retain one." }); }}>{spatialUnlocked ? "Room unlocked" : "Unlock room"}</button>
