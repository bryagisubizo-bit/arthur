...
import { Check, Copy, Globe2, Languages, Search, Star } from "lucide-react";
import { toast } from "sonner";
import { filterLanguages, prepareMultilingualSearch, type LanguageEntry } from "@/lib/languageLibrary";
...
export default function LanguageLibraryPanel({ activeLanguage, setActiveLanguage }: Props) {
  ...
  return <section className="language-library" aria-label="Arthur language library">
    ...
    <div className="language-library-grid">
      ...
      <section className="language-detail-panel"><div className="language-panel-heading"><div><span className="eyebrow">Selected language</span><h3>{selected.name}</h3></div><Globe2 size={20} /></div><p className="language-native">{selected.nativeLabel}</p><dl><div><dt>ISO code</dt><dd>{selected.code.toUpperCase()}</dd></div><div><dt>Writing system</dt><dd>{selected.script}</dd></div><div><dt>Readiness</dt><dd>{selected.readiness === "profile-ready" ? "Local profile preference" : "Language pack or approved provider"}</dd></div></dl><p className="language-boundary">{selected.readiness === "profile-ready" ? "Arthur can retain this as your current language preference. Actual speech and research still follow their own explicit permissions." : "This entry is available for selection and query preparation. Install a local pack or connect and approve a provider separately before Arthur attempts speech, translation, or research in it."}</p><div className="language-actions"><button className="primary-button compact" onClick={() => chooseLanguage(selected)}><Check size={15} /> Use for conversation</button><button className="outline-button" onClick={toggleFavourite}><Star size={15} /> {selectedFavourite ? "Remove favourite" : "Add favourite"}</button></div><div className="language-favourites"><span>Favourites</span>{favourites.map((name) => <button key={name} onClick={() => { const entry = filterLanguages("").find((item) => item.name === name); if (entry) chooseLanguage(entry); }}>{name}</button>)}</div></section>
    </div>
