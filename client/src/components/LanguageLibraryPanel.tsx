...
import { Check, Copy, FileText, Globe2, Languages, Search, Star, Upload } from "lucide-react";
import { toast } from "sonner";
import { createPrivateColloquialDraft, filterLanguages, mergeImportedLanguageReferences, parseIso6393Table, prepareColloquialEntryReview, prepareMultilingualSearch, type ColloquialEntryReview, type ImportedLanguageReference, type LanguageEntry, type PrivateColloquialDraft } from "@/lib/languageLibrary";
...
export default function LanguageLibraryPanel({ activeLanguage, setActiveLanguage }: Props) {
  ...
  const savePrivateDraft = () => {
    try {
      ...
      setExpression(""); setMeaning(""); setRegionalContext(""); setSourceNote(""); setSensitivityNote(""); setReviewPreview(null);
  ...
  return <section className="language-library" aria-label="Arthur language library">
    <header className="language-library-hero"><div><span className="eyebrow">Language library / local catalogue</span><h2>Choose the language before Arthur chooses a route.</h2><p>This bundled catalogue helps you find language names, ISO codes, native labels, and writing systems. Selection is local: it never downloads a pack, turns on the microphone, translates your text, or begins a web search.</p></div><div className="language-library-seal"><Languages size={24} /><b>{catalogue.length.toLocaleString()}</b><span>catalogued<br/>languages</span></div></header>

    <section className="language-catalogue-import"><div><span className="eyebrow">All-language coverage / local identifier table</span><h3>Stage an official ISO 639-3 identifier table on this device.</h3><p>Arthur has a bundled discovery set. Choose an official tab-separated table to add many known language identifiers for local selection and search. The file is read only in this browser session; it is not uploaded, stored, or treated as a language pack or slang source.</p></div><label className="outline-button"><Upload size={15} /> Choose ISO table<input type="file" accept=".tab,.tsv,text/tab-separated-values,text/plain" onChange={(event) => importIdentifierTable(event.target.files?.[0])} /></label><div className="catalogue-import-summary"><b>{importedReferences.length.toLocaleString()}</b><span>additional identifiers staged locally</span></div></section>

    <div className="language-library-grid">
      <section className="language-browser-panel"><div className="language-panel-heading"><div><span className="eyebrow">Discover</span><h3>Search the local library</h3></div><span>{matches.length} matches</span></div><label className="language-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, ISO code, native label, or script" /></label><div className="language-match-list" role="listbox" aria-label="Language matches">{matches.map((entry) => <button key={entry.code} className={`language-match ${selected.code === entry.code ? "selected" : ""}`} onClick={() => chooseLanguage(entry)} role="option" aria-selected={selected.code === entry.code}><span><b>{entry.name}</b><small>{entry.nativeLabel} · {entry.code.toUpperCase()}</small></span><em>{entry.readiness === "profile-ready" ? "profile ready" : "pack / provider"}</em></button>)}</div></section>

      <section className="language-detail-panel"><div className="language-panel-heading"><div><span className="eyebrow">Selected language</span><h3>{selected.name}</h3></div><Globe2 size={20} /></div><p className="language-native">{selected.nativeLabel}</p><dl><div><dt>ISO code</dt><dd>{selected.code.toUpperCase()}</dd></div><div><dt>Writing system</dt><dd>{selected.script}</dd></div><div><dt>Readiness</dt><dd>{selected.readiness === "profile-ready" ? "Local profile preference" : "Language pack or approved provider"}</dd></div><div><dt>Community review</dt><dd>{selected.communityReview}</dd></div><div><dt>Vitality context</dt><dd>{selected.vitalityContext}</dd></div><div><dt>Colloquial content</dt><dd>{selected.colloquialStatus}</dd></div></dl><p className="language-boundary">{selected.readiness === "profile-ready" ? "Arthur can retain this as your current language preference. Actual speech and research still follow their own explicit permissions." : "This entry is available for selection and query preparation. Install a local pack or connect and approve a provider separately before Arthur attempts speech, translation, or research in it."}</p><div className="language-actions"><button className="primary-button compact" onClick={() => chooseLanguage(selected)}><Check size={15} /> Use for conversation</button><button className="outline-button" onClick={toggleFavourite}><Star size={15} /> {selectedFavourite ? "Remove favourite" : "Add favourite"}</button></div><div className="language-favourites"><span>Favourites</span>{favourites.map((name) => <button key={name} onClick={() => { const entry = filterLanguages("", catalogue).find((item) => item.name === name); if (entry) chooseLanguage(entry); }}>{name}</button>)}</div></section>
    </div>

    <section className="community-language-review"><div className="language-panel-heading"><div><span className="eyebrow">Community context / source-governed drafts</span><h3>Do not make slang a stereotype.</h3></div><FileText size={20} /></div><p>Arthur includes no fabricated expressions. Every language can hold a local draft only when its language, regional context, meaning, source or attribution, and sensitivity note are supplied. A review preview never becomes verified content, translation, speech, search, or a reply.</p><div className="community-language-status"><span>Selected language</span><b>{selected.name}</b><span>Review state</span><b>{selected.communityReview}</b></div><div className="colloquial-draft-grid"><label>Expression<input value={expression} onChange={(event) => setExpression(event.target.value.slice(0, 120))} maxLength={120} placeholder="Private draft only" /></label><label>Meaning<input value={meaning} onChange={(event) => setMeaning(event.target.value.slice(0, 240))} maxLength={240} placeholder="Required for review" /></label><label>Region / context<input value={regionalContext} onChange={(event) => setRegionalContext(event.target.value.slice(0, 160))} maxLength={160} placeholder="Required" /></label><label>Sensitivity / use note<input value={sensitivityNote} onChange={(event) => setSensitivityNote(event.target.value.slice(0, 180))} maxLength={180} placeholder="Required for review" /></label><label className="colloquial-source">Source or community attribution<textarea value={sourceNote} onChange={(event) => setSourceNote(event.target.value.slice(0, 240))} maxLength={240} placeholder="Required; not sent anywhere" /></label></div><div className="language-actions"><button className="outline-button" onClick={previewReview}><FileText size={15} /> Prepare review preview</button><button className="outline-button" onClick={savePrivateDraft}><FileText size={15} /> Save private local draft</button><span>{privateDrafts.length} private draft{privateDrafts.length === 1 ? "" : "s"} · not community reviewed</span></div>{reviewPreview && <div className="prepared-language-query needs-input"><b>{reviewPreview.reviewStatus}</b><p>{reviewPreview.language} · {reviewPreview.regionalContext}</p><code>{reviewPreview.expression} — {reviewPreview.meaning}</code><p>Source: {reviewPreview.sourceNote}<br />Sensitivity: {reviewPreview.sensitivityNote}</p></div>}</section>
