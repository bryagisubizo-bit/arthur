...
function planCommandPreview(request: string): CommandPlanPreview {
  ...
  const providerNeeds: Array<[string[], string, string]> = [
    [["research", "search the web", "latest information"], "Web research", "No approved Web research API room is connected. Add a search provider in the API vault to continue."],
    [["calendar", "meeting", "schedule"], "Calendar & productivity", "No approved Calendar & productivity API room is connected. Add a calendar provider to continue."],
    [["play music", "play song", "sing", "make a song"], "Music", "No approved Music API room is connected. Add a music provider in the API vault to continue."],
    [["home assistant", "lights", "thermostat", "smart home"], "Smart home", "No approved Smart home API room is connected. Add a Home Assistant room in the API vault to continue."],
    [["analyse screen", "analyze screen", "analyse file", "analyze file"], "Files & documents", "No approved Files & documents room is connected. Add an analysis provider and select the source to continue."],
  ];
  const missingProvider = providerNeeds.find(([phrases]) => phrases.some((term) => input.includes(term)));
  if (missingProvider) return { intent: "resource unavailable", summary: "Arthur found the required capability, but no approved resource is connected.", command: "—", risk: "blocked", confirmation: false, allowed: false, missingRoom: missingProvider[1], reason: missingProvider[2] };
    ...
    <main className={`arthur-app colour-${colourMode}`}>
      ...
      <section className={`command-canvas ${section === "persona" ? "persona-active" : ""}`}>
        ...
        {section === "command" && <>
          ...
          {commandPlan && <section className={`command-plan ${commandPlan.risk}`} aria-live="polite"><div className="plan-emblem">{commandPlan.missingRoom ? <AlertTriangle size={20} /> : <TerminalSquare size={20} />}</div><div className="plan-copy"><span className="eyebrow">Reviewed command plan / {commandPlan.risk}</span><h3>{commandPlan.summary}</h3><code>{commandPlan.command}</code>{commandPlan.reason && <p>{commandPlan.reason}</p>}{commandPlan.missingRoom && <div className="missing-resource-alert" role="alert"><AlertTriangle size={17} /><div><b>Missing API resource: {commandPlan.missingRoom}</b><span>Arthur has not sent a request or tried a fallback. Add and test an approved provider room before continuing.</span></div><button className="primary-button compact" onClick={() => setSection("api")}>Add API resource <KeyRound size={15} /></button></div>}</div><div className="plan-actions">{commandPlan.confirmation ? <StatusPill tone="amber">Approval required</StatusPill> : commandPlan.allowed ? <StatusPill tone="green">Read-only diagnostic</StatusPill> : <StatusPill tone="amber">Not available</StatusPill>}<button className="outline-button" onClick={() => toast(commandPlan.allowed ? "The production app would show this exact command and follow its permission policy." : commandPlan.missingRoom ? `The ${commandPlan.missingRoom} room must be added and tested before Arthur can continue.` : "Arthur will retain no raw command text in its audit record.")}>{commandPlan.allowed ? "Inspect policy" : commandPlan.missingRoom ? "Why resource is needed" : "Why blocked?"}</button></div></section>}
